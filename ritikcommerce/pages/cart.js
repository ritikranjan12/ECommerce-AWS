...
export default function CartPage() {
  ...
  // Define cartProductQuantities to track product quantities
  const cartProductQuantities = {};
  cartProducts.forEach((productId) => {
    cartProductQuantities[productId] =
      (cartProductQuantities[productId] || 0) + 1;
  });
  ...
  if (isSuccess) {
    ...
    return (
      <>
        <Center style={{marginTop:'100px', minHeight: '100vh'}}>
          <ColumnsWrapper>
            <Box>
              <h2>Cart</h2>
              {!cartProducts?.length && (
              ...
              {products?.length > 0 && (
                <Table total={total}>
                  ...
                  <tbody>
                    {Object.keys(cartProductQuantities).map((productId) => {
                      const product = products.find((p) => p.id === productId);
                      if (product) {
                        return (
                          <tr key={product.id}>
                            ...
                            <td>
                              <Button onClick={() => lessOfThisProduct(product.id)}>-</Button>
                              <QuantityLabel>{cartProductQuantities[product.id]}</QuantityLabel>
