...
export default function Nav({show}) {
  const inactiveLink = 'flex gap-1 p-1 my-2';
  const activeLink = inactiveLink+' bg-white rounded-full my-2';
  const inactiveIcon = 'w-6 h-6 my-2';
  const activeIcon = inactiveIcon + ' text-black ml-2 mr-2';
  ...
  return (
    ...
    <aside className={(show?'left-0':'-left-full')+" top-0  p-4 fixed w-full bg-blue-500 h-screen md:static md:w-auto transition-all"}>
      ...
      <nav className="flex flex-col gap-2">
        <Link href={'/'} className={pathname === '/' ? activeLink : inactiveLink}>
          ...
          <span className="my-2">Dashboard</span>
        ...
        <span className="my-2">Categories</span>
        </Link>
